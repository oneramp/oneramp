...
const ContractInteractions_1 = __importDefault(require("./subclasses/ContractInteractions"));
const SubgraphInteractions_1 = __importDefault(require("./subclasses/SubgraphInteractions"));
/**
 *
...
class ToucanClient {
    ...
    constructor(network, provider, signer) {
         ...
         */
        this.retire = async (amount, tco2Address) => {
            if (!this.signer)
         ...
         * @param tco2Address address of the TCO2 token to retire* @returns retirement transaction
         */
        this.retireAndMintCertificate = async (retirementEntityName, beneficiaryAddress, beneficiaryName, retirementMessage, amount, tco2Address) => {
            if (!this.signer)
                throw new Error("No signer set");
            const signer = this.signer;
            return this.contractInteractions.retireAndMintCertificate(retirementEntityName, beneficiaryAddress, beneficiaryName, retirementMessage, amount, tco2Address, signer);
        };
        /**
         *
         ...
         * @param tco2Address address of the TCO2 token to retire* @returns retirement transaction
         */
        this.retireFrom = async (amount, address, tco2Address) => {
            if (!this.signer)
                throw new Error("No signer set");
            const signer = this.signer;
            return this.contractInteractions.retireFrom(amount, address, tco2Address, signer);
        };
        /**
         *
         ...
         * @returns
         */
        this.getDepositCap = async (tco2Address) => {
            const signerOrProvider = this.signer ? this.signer : this.provider;
            if (!signerOrProvider)
                throw new Error("No signer or provider set");
            return this.contractInteractions.getDepositCap(tco2Address, signerOrProvider);
        };
        /**
         *
         ...
         */
        this.getAttributes = async (tco2Address) => {
            const signerOrProvider = this.signer ? this.signer : this.provider;
            if (!signerOrProvider)
                throw new Error("No signer or provider set");
            return this.contractInteractions.getAttributes(tco2Address, signerOrProvider);
        };
        /**
         *
         ...
         */
        this.getTCO2Remaining = async (tco2Address) => {
            const signerOrProvider = this.signer ? this.signer : this.provider;
            if (!signerOrProvider)
                throw new Error("No signer or provider set");
            return this.contractInteractions.getTCO2Remaining(tco2Address, signerOrProvider);
        };
        // --------------------------------------------------------------------------------
        // --------------------------------------------------------------------------------
         ...
         */
        this.depositTCO2 = async (pool, amount, tco2Address) => {
            if (!this.signer)
                throw new Error("No signer set");
            const signer = this.signer;
            return this.contractInteractions.depositTCO2(pool, amount, tco2Address, signer);
        };
        /**
         *
         ...
         * @returns boolean
         */
        this.checkEligible = async (pool, tco2) => {
            const signerOrProvider = this.signer ? this.signer : this.provider;
            if (!signerOrProvider)
                throw new Error("No signer or provider set");
            return this.contractInteractions.checkEligible(pool, tco2, signerOrProvider);
        };
        /**
         *
         ...
         */
        this.calculateRedeemFees = async (pool, tco2s, amounts) => {
            const signerOrProvider = this.signer ? this.signer : this.provider;
            if (!signerOrProvider)
                throw new Error("No signer or provider set");
            return this.contractInteractions.calculateRedeemFees(pool, tco2s, amounts, signerOrProvider);
        };
        /**
         *
         ...
         */
        this.redeemMany = async (pool, tco2s, amounts) => {
            if (!this.signer)
                throw new Error("No signer set");
            const signer = this.signer;
         ...
         * @returns redeem transaction
         */
        this.redeemAuto = async (pool, amount) => {
            if (!this.signer)
                throw new Error("No signer set");
            const signer = this.signer;
         ...
         */
        this.redeemAuto2 = async (pool, amount) => {
            if (!this.signer)
                throw new Error("No signer set");
            const signer = this.signer;
         ...
         * @returns BigNumber representing the remaining space
         */
        this.getPoolRemaining = async (pool) => {
            const signerOrProvider = this.signer ? this.signer : this.provider;
            if (!signerOrProvider)
                throw new Error("No signer or provider set");
         ...
         */
        this.getScoredTCO2s = async (pool) => {
            const signerOrProvider = this.signer ? this.signer : this.provider;
            if (!signerOrProvider)
                throw new Error("No signer or provider set");
         ...
         * @returns boolean
         */
        this.checkIfTCO2 = async (address) => {
            const signerOrProvider = this.signer ? this.signer : this.provider;
            if (!signerOrProvider)
                throw new Error("No signer or provider set");
         ...
         */
        this.autoOffsetUsingPoolToken = async (pool, amount) => {
            if (!this.signer)
                throw new Error("No signer set");
            const signer = this.signer;
            return this.contractInteractions.autoOffsetUsingPoolToken(pool, amount, signer);
        };
        /**
         *
         ...
         */
        this.autoOffsetUsingSwapToken = async (pool, amount, swapToken) => {
            if (!this.signer)
                throw new Error("No signer set");
            const signer = this.signer;
            return this.contractInteractions.autoOffsetUsingSwapToken(pool, amount, swapToken, signer);
        };
        /**
         *
         ...
         * @param amount amount of CO2 tons to offset* @returns offset transaction
         */
        this.autoOffsetUsingETH = async (pool, amount) => {
            if (!this.signer)
                throw new Error("No signer set");
            const signer = this.signer;
         ...
         */
        this.calculateNeededTokenAmount = async (pool, amount, swapToken) => {
            const signerOrProvider = this.signer ? this.signer : this.provider;
            if (!signerOrProvider)
                throw new Error("No signer or provider set");
            return this.contractInteractions.calculateNeededTokenAmount(pool, amount, swapToken, signerOrProvider);
        };
        /**
         *
         ...
         */
        this.calculateNeededETHAmount = async (pool, amount) => {
            const signerOrProvider = this.signer ? this.signer : this.provider;
            if (!signerOrProvider)
                throw new Error("No signer or provider set");
            return this.contractInteractions.calculateNeededETHAmount(pool, amount, signerOrProvider);
        };
        // --------------------------------------------------------------------------------
        ...
        this.fetchUserRetirements = async (walletAddress, first = 100, skip = 0) => {
            return this.subgraphInteractions.fetchUserRetirements(walletAddress, first, skip);
        };
        // --------------------------------------------------------------------------------
        ...
        this.fetchUserRedeems = async (walletAddress, pool, first = 100, skip = 0) => {
            return this.subgraphInteractions.fetchUserRedeems(walletAddress, pool, first, skip);
        };
        // --------------------------------------------------------------------------------
        ...
        this.provider = provider;
        this.signer = signer;
        this.contractInteractions = new ContractInteractions_1.default(network);
        this.subgraphInteractions = new SubgraphInteractions_1.default(network);
    }
}
exports.default = ToucanClient;
